...
var modal_product_detail = document.getElementById('modal-product-detail');
...
function validateSignin() {
      ...
      //? Fullname validate
      if (fullname == '' || fullname.length < 7 || checkForSpecialChar(fullname) == true) {
      ...
      //? Username validate
      if (username == '' || username.length < 5 || checkForSpecialChar(username) == true) {
      ...
      //? Phone validate
      if (phone == '' || phone.length > 15 ||phone.length < 5 || checkForSpecialCharForPhoneNumber(phone) == true) {
...
function login () {
      ...
      if(validateLogin() == true) {
            ...
            if(i < userlist.length) { //! Check username
                  ...
                  if(userlist[i].password == password) { //! Check password
                        ...
                        if(userlist[i].value == 100) {
                              ...
                              customAlert('Bạn đã đăng nhập thành công','success');
                              setTimeout(function(){
                                    window.location.href = 'UserPage/index.html';   //! Link đến web USER
                              },500)
                        }
...
var PageProduct = document.getElementById('numberOfPageProduct')       //! khai báo 
...
function showListPageProductStranger() {
      let numberOfPageProduct = divideProductPage('product').length; //? = 3
      let PageProductTemp = '';
...
function showListPageProductUser() {
      ...
      let PageProductTemp = '';
      for(let i = 0; i < numberOfPageProduct; i++) {
            PageProductTemp += '<button onclick="showProductUser('+i+'),scrollToTop()">'+(i+1)+'</button>'
      }
      PageProduct.innerHTML = PageProductTemp;
}
...
function showProductAdmin() {
      ...
      for (let item of productArray) {      
            tableProduct += '<tr><td>'+item.productId+'</td><td><img src="'+item.img+'" alt=""></td><td class="table-name">'+item.name+'</td><td class="table-price">'+currency(item.price)+'</td><td><button class="option-btn-admin" onclick="deleteProduct('+item.productId+')">Xóa</button><br><button class="option-btn-admin" onclick="changeProductAdmin('+item.productId+')">Sửa</button></td></tr>';
      }
...
//? CART | Giỏ hàng
function showQuantityOfCart() {
      let num = document.getElementById('cart-quantity');
...
function showCart() {
      ...
      if(cartUser == null) {
            var contentCartTable = document.getElementById('cart-table');
            var totalPriceCart = document.getElementById('totalprice-cart');
...
function showBillAdmin() {
      if(JSON.parse(localStorage.getItem('bill')).length == 0) {
...
function searchUser() {
      ...
      for (let i = 0; i < productArray.length; i++) {
            if(productArray[i].name.toLowerCase().indexOf(search) !== -1) {
                  searchProductTemp += '<div class="search-product" onclick="productDetail('+productArray[i].productId+')"><img src="'+productArray[i].img+'" alt="" class="search-product-img"><p class="search-product-name">'+productArray[i].name+'</p><p class="search-product-price">'+currency(productArray[i].price)+'</p></div>'
            } 
      }
...
function searchStranger() {
      ...
      for (let i = 0; i < productArray.length; i++) {
            if(productArray[i].name.toLowerCase().indexOf(search) !== -1) {
                  searchProductTemp += '<div class="search-product" onclick="customAlert(\'Bạn phải đăng nhập để mua hàng\',\'warning\')"><img src="'+productArray[i].img+'" alt="" class="search-product-img"><p class="search-product-name">'+productArray[i].name+'</p><p class="search-product-price">'+currency(productArray[i].price)+'</p></div>'
            } 
      }
      search_product.innerHTML = searchProductTemp;
      if(search == '') {
